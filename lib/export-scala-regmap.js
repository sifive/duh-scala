...
const generateOMRegisterMap = (addressUnitBits, addressBlock) => {
  const registerMapElements = addressBlock.registers.map((register) => {
    const regFieldSeqElements = (register.fields || []).map(field => {
      ...
      return field.bitOffset + ' -> ' +
        generateRegisterField(field, 'Bool()', access, field.resetValue);
    });

    const registerDesc = register.description
    ...
    const addressOffset = (addressUnitBits === 8)
      ? register.addressOffset
      : `${register.addressOffset} * ${addressUnitBits / 8}`;
  ...
  return (`RegFieldAddressBlock(
  AddressBlockInfo(
    "${addressBlock.name}",
    addressOffset = ${addressBlock.baseAddress},
    range = ${addressBlock.range},
    width = ${addressBlock.width}
  ),
...
module.exports = {exportRegmap, generateOMRegisterMap, padFieldsMethod};

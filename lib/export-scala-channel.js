...
const masterWiringGen = comp => e =>
  ...
  def out(x: Int) = {
    val y = IO(Decoupled(${chisel.convertType(findPort(comp, e).wire)}))
...
const slaveWiringGen = comp => e =>
  ...
  def in(x: Int) = {
    val y = IO(Flipped(Decoupled(${chisel.convertType(findPort(comp, e).wire)})))
      y.suggestName("${e.name}")
      (y, Nil)
    }
  }
...
${comp.busInterfaces
    ...
    .map(e => `    val ${e.name}_queue = Module(new Queue(${chisel.convertType(findPort(comp, e).wire)}, 4))`)
    .join('\n')
}

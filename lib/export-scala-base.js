...
const blackWire = {
  'out': outConnect,
  'analog out':  outConnect,
  'in': inConnect,
  'analog in': inConnect,
  'analog inout': inoutConnect
};
...
const perBundlePortWiring = () => e => {
  ...
  if (e.wire.direction === 'out' || e.wire.direction === 'analog out') {
    return h1 + ' := ' + h2;
  } else if (e.wire.direction === 'analog inout') {
    return h1 + ' <> ' + h2;
  }
  return h2 + ' := ' + h1;
};
...
const renameWires = comp => {
  ...
  comp.model.ports.foreach((port) => {
    if (isAnalog(port.wire)) {
      port.wire.originalName = port.wire.name
      port.wire.name = port.wire.name + '_ANALOG'
    }
    port.wire.originalName = port.wire.name
  })
}

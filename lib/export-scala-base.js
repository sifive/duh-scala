...
const memoryMappedBuses = {AXI4: 1, AXI4Lite: 1, APB4: 1, AHBLite: 1};
...
const memorySlaves = comp => comp
  ...
  .filter(e => (memoryMappedBuses)[e.busType.name]);
...
object N${comp.name}TopParams {
  def defaults(
${indent(4)(memorySlaves(comp).map(e => e.name + '_base: BigInt').join(',\n'))}
  ) = N${comp.name}TopParams(
    blackbox = ${comp.name}Params(),
${indent(4)(
    comp.busInterfaces
      .map(e => `${e.name}Params = P${e.name}Params(${
        (memoryMappedBuses[e.busType.name] && (e.interfaceMode === 'slave'))
          ? `base = ${e.name}_base` : ''
      })`)
      .join(',\n')
  )}
...
${indent(2)(memorySlaves(comp).map(e => e.name + '_base: BigInt').join(',\n'))}
  ...
  comp.name
}TopParams.defaults())) +: up(BlockDescriptorKey, site)
})

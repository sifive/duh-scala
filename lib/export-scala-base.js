...
case class OM${comp.name.toUpperCase()}[T](
  base: ${comp.name}Params,
  user: T,
  memoryRegions: Seq[OMMemoryRegion] = Nil, //TODO
  interrupts: Seq[OMInterrupt] = Nil, //TODO
  _types: Seq[String] = Seq("OM${comp.name.toUpperCase()}", "OMDevice", "OMComponent", "OMCompoundType")
) extends OMDevice

class N${comp.name}TopLogicalTreeNode[T](${comp.name}: N${comp.name}TopBase, userOM: T) extends LogicalTreeNode(() => Some(${comp.name}.imp.device)) {
  override def getOMComponents(resourceBindings: ResourceBindings, components: Seq[OMComponent]): Seq[OMComponent] = {
    DiplomaticObjectModelAddressing.getOMComponentHelper(
      resourceBindings, (resources) => {
        Seq(OM${comp.name.toUpperCase()}(
          user = userOM,
          base = ${comp.name}.c.blackbox
        ))
      })
  }
}
...
class N${comp.name}TopBase(val c: N${comp.name}TopParams)(implicit p: Parameters)
 ...
 with HasLogicalTreeNode {
  ...
  def logicalTreeNode: LogicalTreeNode = new N${comp.name}TopLogicalTreeNode(this, userOM)
